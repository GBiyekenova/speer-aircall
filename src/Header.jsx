...
import { INBOUND_UNARCHIVED, ALL_UNARCHIVED, ALL_ARCHIVED } from "./App.jsx";

const Header = () => {
  const { callType, setCallType } = useContext(GeneralContext);

  const showInboundCalls = () => {
    setCallType(INBOUND_UNARCHIVED);
  };

  const showAllCalls = () => {
    setCallType(ALL_UNARCHIVED);
  };

  const showArchivedCalls = () => {
    setCallType(ALL_ARCHIVED);
  };
  ...
  return (
    <header className="header flex-row">
      ...
      <div className="flex-row space-even background-gray">
        <button
          onClick={() => {
            showInboundCalls();
          }}
          className={
            (callType[0] === "inbound" ? "clicked" : "regular") + " general-btn"
          }
        >
          Inbox
        </button>
        ...
        <button
          ...
          className={
            (callType[0] === "all" ? "clicked" : "regular") + " general-btn"
          }
        >
          All Calls
        </button>
